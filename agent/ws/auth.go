...
func Authenticate(
	...
	if msg.Type != "auth" {
		writeAuthFailed(
			conn,
			"first websocket message must be auth",
		)
	...
	device, ok := deviceStore.FindDeviceByToken(
		request.Token,
	)
...
func writeAuthSuccess(
	conn *websocket.Conn,
	device *store.Device,
) {
	payload, err := protocol.EncodePayload(
		protocol.AuthSuccess{
